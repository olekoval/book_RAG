...
def find_best_match_keyword_search(query, db_records):
    """
    Виконує пошук найкращого збігу за ключовими словами між запитом та списком записів.

    Функція порівнює набір слів із запиту з набором слів у кожному записі списку,
    використовуючи операцію перетину множин для підрахунку кількості спільних слів.

    Args:
        query (str): Текстовий запит користувача для пошуку.
        db_records (list of str): Список текстових рядків (корпус документів), 
            серед яких проводиться пошук.

    Returns:
        tuple: Кортеж, що містить:
            - best_score (int): Найвища кількість спільних ключових слів, знайдена серед записів.
            - best_record (str або None): Текст запису з найбільшою кількістю збігів.
    """
    best_score = 0
    best_record = None
...
with open("./test_out/response.txt", "w", encoding="utf-8") as f:
